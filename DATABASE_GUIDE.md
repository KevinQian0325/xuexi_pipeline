...
前端优先读取 `程序运行文件夹/爬取日志/*.json` 展示客户可见日志。运行数据库更适合内部管理页面使用。
...
3. 重跑时会优先复用已有产物：已有 `文本.docx` 会直接标记完成，已有 `音频.wav` 会从 ASR 开始，已有 `视频.mp4` 会从音频提取开始。
4. 不建议手动删除 `videos` 表，否则会丢失历史处理状态。
5. 如果需要重新处理某条视频，可以把该视频的 `status` 改回 `NEW` 或非 `DOCX_DONE` 状态。
6. 如果只想重跑失败视频，可以筛选 `status = 'FAILED'` 后重置状态。
